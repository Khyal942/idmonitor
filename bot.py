...
DATABASE_FILE = os.environ.get("users_data.db")
    ...
    try:
        while monitoring_active:
            ...
            for user_id, username, last_known_username, not_found in users:
                try:
                    # Debounce: Add a small delay to prevent rapid re-triggering
                    ...
                    if current_username is None:
                        if user_id not in notified_users or notified_users[user_id] != "missing":
                            ...
                            await send_notification(chat_id,
                                                    f"User ID {user_id} ({user_name}) currently does not have a username set.")
                        update_user(user_id, None, True)
                        continue

                    # Check for username change
                    if current_username != last_known_username:
                        # Ensure this is not a repeat of the same change
                        if (user_id not in notified_users or notified_users[user_id] != current_username):

                            print(f'User ID {user_id} ({user_name}) changed username from @{last_known_username} to @{current_username}.')
                            await send_notification(chat_id,
                                                    f'User ID {user_id} ({user_name}) changed username from @{last_known_username} to @{current_username}.')
...
async def delete_user_command(_, update):
    chat_id = update.chat.id  # Get the chat ID where the command was issued
    users = get_users(chat_id)  # Pass the chat_id to get_users
    for user_id, username, last_known_username, not_found in users:
